...
chirps_years <- as.numeric(gsub(
  "chirps-v2\\.0\\.(\\d{4})\\.days_p05\\.nc",
  "\\1",
  chirps_files
))

message(glue::glue(
  "Found CHIRPS data for years: {paste(sort(chirps_years), collapse = ', ')}"
))
...
TEST_DATE <- as.Date(
  glue::glue("{TEST_YEAR}-{TEST_DAY_OF_YEAR}"),
  format = "%Y-%j"
)
message(glue::glue(
  "Selected test date: {TEST_DATE} (day {TEST_DAY_OF_YEAR} of {TEST_YEAR})"
))
...
# Test CHIRPS local
chirps_local <- run_benchmark(
  "CHIRPS - Local",
  extract_chirps_local,
  TEST_YEAR,
  TEST_DAY_OF_YEAR,
  design_points
)
...
# Test CHIRPS remote
chirps_remote <- run_benchmark(
  "CHIRPS - Remote",
  extract_chirps_remote,
  TEST_YEAR,
  TEST_DAY_OF_YEAR,
  design_points
)
...
if (chirps_local$success && chirps_remote$success) {
  message("\n--- Data Comparison ---")
  chirps_diff <- max(
    abs(chirps_local$result$precip_mm_day - chirps_remote$result$precip_mm_day),
    na.rm = TRUE
  )
  message(glue::glue(
    "Maximum difference in precip values: {round(chirps_diff, 6)} mm/day"
  ))
}
